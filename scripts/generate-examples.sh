...
python -m snipinator.cli \
  ...
  --args '{"example_type": "npm-module", "module_name": "chatgpt2graph"}' \
...
python -m snipinator.cli \
  -t "${PROJ_PATH}/examples/simple_example.sh.jinja2" \
  --args '{"example_type": "docker-local", "docker_image_name": "my-chatgpt2graph-image"}' \
  --warning-header "${CUSTOM_WARNING%.}" \
  --rm \
  --force \
  --create \
...
python -m snipinator.cli \
  -t "${PROJ_PATH}/examples/simple_example.sh.jinja2" \
  --args '{"example_type": "docker-remote", "docker_image_name": "ghcr.io/realazthat/chatgpt2graph:v'"${LAST_VERSION}"'"}' \
  --warning-header "${CUSTOM_WARNING%.}" \
  --rm \
  --force \
  --create \
